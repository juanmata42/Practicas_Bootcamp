...
function miraloquetengo() {
    for (let i = 0; i < flights.length; i++) {
        console.log(
            `El vuelo con origen\: ${flights[i]["from"]}, y destino\: ${
                flights[i]["to"]
...
function extrainfo() {
    ...
    console.log(
        "El número de vuelos que realizan escalas es " + contadorEscalas() + "."
    );
    console.log(
...
function useradmin() {
    ...
    let tipoU = prompt(
        "Por favor, identifícate como USER o ADMIN"
    ).toUpperCase();
    if (tipoU === "ADMIN") {
        adminchosen();
    }
...
function userchosen() {
    ...
    let ordenuser = prompt(
        "Introduce un precio para buscar un vuelo con ese coste. \nIntroduce caro o barato para ver el vuelo que corresponda a ello.\nIntroduce comprar para adquirir un vuelo."
    );
    if (ordenuser === "caro") {
        let getMax = listaprecios.indexOf(Math.max(...listaprecios));
        alert(
            `Vuelo con origen\: ${flights[getMax]["from"]}, y destino\: ${
                flights[getMax]["to"]
            } tiene un coste de ${flights[getMax]["cost"]}€ y ${escala(
                flights[getMax]["scale"]
            )}. Su ID es ${flights[getMax]["id"]}.`
        );
        console.log(
            `Vuelo con origen\: ${flights[getMax]["from"]}, y destino\: ${
                flights[getMax]["to"]
        ...
        alert(
            `Vuelo con origen\: ${flights[getMin]["from"]}, y destino\: ${
                flights[getMin]["to"]
            } tiene un coste de ${flights[getMin]["cost"]}€ y ${escala(
                flights[getMin]["scale"]
            )}. Su ID es ${flights[getMin]["id"]}.`
        );
        console.log(
            `Vuelo con origen\: ${flights[getMin]["from"]}, y destino\: ${
                flights[getMin]["to"]
        ...
        let compra = prompt(
            "Introduce el número de ID del vuelo que deseas adquirir"
        );
        ...
        if (listaprecios.includes(ordenuser)) {
            ...
            alert(
                `Vuelo con origen\: ${flights[getExact]["from"]}, y destino\: ${
                    flights[getExact]["to"]
                } tiene un coste de ${flights[getExact]["cost"]}€ y ${escala(
                    flights[getExact]["scale"]
                )}. Su ID es ${flights[getMax]["id"]}.`
            );
            console.log(
                `Vuelo con origen\: ${flights[getExact]["from"]}, y destino\: ${
            ...
            alert(
                "No hay ningún vuelo con ese precio o la palabra introducida no es correcta.\nVuelve a intentarlo, anda."
            );
            userchosen();
        }
    }
...
function createf() {
    if (flights.length < 16) {
        let newFlightInput = prompt(
            "Añade los datos del nuevo vuelo siguiendo este esquema: ID, destino, origen, coste, escala. Ejemplo: 13, Roma, La paz, 560, si"
        ).split(", ");
        if (newFlightInput) {
...
function deleteflight() {
    if (flights.length > 1) {
        let idToDel = parseInt(
            prompt("Introduce el id del vuelo que desea borrar.")
        );
        ...
        alert("No quedan vuelos. Por favor, crea alguno o cierra el programa.");
        adminchosen();
    }
}
