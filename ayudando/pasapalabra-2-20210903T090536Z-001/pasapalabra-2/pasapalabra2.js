...
function restaPalabra() {
    palabras.textContent -= 1;
};

function changeColor() {
  for(let i = 0; i < letras.length; i++){
      if(letras[i].textContent === seleccion[sorteo].letter && seleccion[sorteo].status === 'a'){
          letras[i].style.backgroundColor = 'chartreuse'
      }else if(letras[i].textContent === seleccion[sorteo].letter && seleccion[sorteo].status === 'f'){
        letras[i].style.backgroundColor = 'rgb(209, 10, 10)';
      }else if(letras[i].textContent === seleccion[sorteo].letter && seleccion[sorteo].status === 'p'){
...
function acabarPartida() {
    tiempo.textContent = 150;
    alert(`Has acertado: ${acertadas}, has fallado: ${falladas}`);
    changeColor();
    start = 0;
    end = 3;
    acertadas = 0;
...
function rosco() {
    if(preguntasRealizadas < 27){
        seleccion = questions.slice(start, end);
        sorteo = Math.floor(Math.random() * 3);
        pregunta.textContent = (seleccion[sorteo].question);
    }else{
        segundaVuelta();
        console.log('ola')
    }
...
function trueFalse() {
    if( responder.value !== seleccion[sorteo].answer){
        questions[start + sorteo].status = 'f';
        falladas += 1;
        fallos.textContent = falladas;
        changeColor();
        nextQuestion();
        rosco()
    }else if(responder.value === seleccion[sorteo].answer){
        questions[start + sorteo].status = 'a';
        acertadas += 1;
        aciertos.textContent = acertadas;
        changeColor();
        nextQuestion();
        rosco();
    };
...
function pasaSiguiente() {
    questions[start + sorteo].status = 'p';
    pasadas += 1;
    changeColor();
    nextQuestion();
    rosco();
...
//onclick functions de los botones
buttonPlay.onclick = () => {
    if(empezar === false){
        empezar = true;
        acabar = true;
...
ok.onclick = () => {
    if(empezar === true && preguntasRealizadas !== 27){
        preguntasRealizadas++;
        restaPalabra();
        trueFalse();
    };
...
//Formulario no actualize pagina
form.addEventListener('submit', (ev) => {
    ev.preventDefault();
});
//Event al pulsar teclas
window.addEventListener('keydown', (ev) => {
    if(ev.code === "Space" && empezar === true && preguntasRealizadas !== 27){
        pasaSiguiente();
    };
});