...
// function segundoRosco(){
//     pregunta.textContent = pasapalabrasNuevaVuelta[index].question
// }
// function incremteI(i){
//     let newI;
//     if(i === 26){
//         newI = 0;
//     }else{
...
//hacer la suma de palabras pasadas + aciertos + fallos para condicion
...
function newIndex(){
    for(let i = 0; i < pasapalabra.length; newI(i)){
        if(pasapalabra[i].status === 'p'){
            index = i
            i = 27;
        }
    }
...
function proximaOkVuelta(){
    if( segundaVuelta === true){
        newIndex()
    }
}

function proximaPasaVuelta(){
    if(segundaVuelta === true){
        newIndex()
    }
}
